...
class DupMAEForPretraining(nn.Module):
    ...
    def decoder_mlm_loss(
        ...
        hiddens = torch.cat(
            [sentence_embedding, decoder_embedding_output[:, 1:]], dim=1
        )

        query_embedding = sentence_embedding.expand(
            hiddens.size(0), hiddens.size(1), hiddens.size(2)
        )
    ...
    def forward(
        ...
        # print("WE ARE IN THE FORWARD FUNCTION" * 8)
        ...
        mlm_loss = self.decoder_mlm_loss(
            cls_hiddens, decoder_input_ids, decoder_attention_mask, decoder_labels
        )

        ot_embedding = self.sparse_embedding(
            lm_out.logits[:, 1:], encoder_attention_mask[:, 1:]
        )
